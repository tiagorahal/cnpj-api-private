...
# ============ Limites de recursos ============
MAX_RAM_GB = 35     # Ajuste conforme sua RAM real!
MAX_SWAP_GB = 5
CHECK_RAM_EACH = 10_000 # Checa RAM a cada X registros processados
# =============================================

gstep = 2000000 #quantidade de registros por loop (ajuste para menos se travar)
...
bMemoria = False #se tiver menos de 16GB de RAM, colocar bMemoria=False
...
def ram_ok():
    ...
    if ram_gb >= MAX_RAM_GB or swap_gb >= MAX_SWAP_GB:
        print(f"ALERTA: RAM usada: {ram_gb:.2f}GB | SWAP usada: {swap_gb:.2f}GB (limite RAM: {MAX_RAM_GB}GB, SWAP: {MAX_SWAP_GB}GB)")
    return (ram_gb < MAX_RAM_GB) and (swap_gb < MAX_SWAP_GB)

def wait_for_ram():
    while not ram_ok():
        print("Memória/swap alta, aguardando liberar recursos... (30s)")
        time.sleep(30)
...
# ... (demais funções de normalização, dicAbreviaturas, etc. MANTENHA COMO ESTÃO!)
...
def normalizaEndereco(enderecoIn, ignoraEnderecoSoComNumeros = True, ignoraEnderecoSemNumeros = True):
    if 1:
        enderecoAux = re.sub(r'([0-9]+)[.]([0-9]+)', '\\1\\2', enderecoIn).upper() #remove ponto de n�mero
        enderecoAux = re.sub(r'([A-Z])(\d)', '\\1 \\2', enderecoAux) #separa letra de d�gito
        enderecoAux = re.sub(r'(\d)([A-Z])', '\\1 \\2', enderecoAux) #separa digito de letra
    else: #fazendo compile fica mais lento!
        enderecoAux = regex1.sub(r'\\1\\2', enderecoIn).upper() #remove ponto de n�mero
        enderecoAux = regex2.sub(r'\\1 \\2', enderecoAux) #separa letra de d�gito
    ...
    for k,pedaco in enumerate(lendereco):
        ...
        if pedaco in dicAbreviaturas:
            if len(pedaco)>1 or k<=1: #alguns casos como R ou Q s� se for no come�o do nome
    ...
    palavrasOrdenadas = sorted(list(palavras)) #<---------------------Ordena��o, pode ser removido
    if ignoraEnderecoSemNumeros: #h� muitos endere�os sem n�mero
        if not numeros:
            return ''
    ...
    if ignoraEnderecoSoComNumeros:
        if endereco.translate(tremovenumerosespacos) == '': #remove numeros e espa�o
            return ''
        else:
            return endereco
    else:
...
def ajustaTelefone(telefoneIn):
    if telefoneIn is None or telefoneIn=='' or telefoneIn == '0 0' or \
       telefoneIn[-7:] in ('0000000','1111111','2222222','3333333','4444444','5555555','6666666','7777777','8888888','9999999'):
        return ''
    telefoneIn = ' '.join(telefoneIn.split()).strip() #remove espa�os duplicados
    if ' ' in telefoneIn:
        pos = telefoneIn.find(' ')
...
# ------------------------------------------
def baixa_enderecos_cnpj(bpergunta, conBaseCompleta, conEnderecoNormalizado):
    ...
    queryBase = '''
                ...
                IIF(t.uf<>'EX', t.uf , tpais.descricao ) as uf
    ...
    while True:
        ...
        dftmptable.to_sql('endereco', conEnderecoNormalizado, if_exists='append')
        gc.collect()
        inicio += kregistros
...
def baixa_telefone_cnpj(bpergunta, conBaseCompleta, conTelefone):
    ...
    queryBase = '''
                SELECT ROWID, cnpj, situacao_cadastral as situacao, ddd1, telefone1, ddd2, telefone2, ddd_fax, fax
                FROM estabelecimento t
            '''
    inicio = 0
    kregistros = gstep   
    ...
    while True:
        ...
        if lista:
            ...
            dftmptable.to_sql('telefone', conTelefone, if_exists='append', index=False)
        gc.collect()
        inicio += kregistros
        if inicio>numeroDeRegistros:
            break
...
def baixa_email_cnpj(bpergunta, conBaseCompleta, conTelefone):
    ...
    queryBase = '''
                SELECT ROWID, cnpj, situacao_cadastral as situacao, correio_eletronico as email
                FROM estabelecimento t
            '''
    inicio = 0
    kregistros = gstep
    ...
    while True:
        ...
        if lista:
            ...
            dftmptable.to_sql('email', conTelefone, if_exists='append', index=False)
        gc.collect()
        inicio += kregistros
        if inicio>numeroDeRegistros:
            break
...
# ... (demais funções e dicAbreviaturas permanecem iguais)
