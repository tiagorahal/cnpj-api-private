...
"""
rictom
2022-10-21 - alterado, removido filtro de empresas ativas para reduzir tempo de execu��o.
teste com dask na rotina de endere�os, tem desempenho igual ao pandas (um pouco mais lento)

2022-11-24 c�digo alterado para usar sqlite3
"""
...
def agrupa_cnpj_por_tipo(tipo, con):
    ...
    #2022-10-21 filtro ser� ignorado, para reduzir tempo de execu��o
    if tipo=='endereco':
...
# def removeTabelasTemporarias(bpergunta=True): #usando sqlite3, n�o d� para usar esta fun��o
#     con = sqlalchemy.create_engine(f"sqlite:///{camDBSaida}", execution_options={"sqlite_raw_colnames": True})
#     insp = sqlalchemy.inspect(con)
#     nomes = insp.get_table_names()
#     tabelasAApagar = [n for n in nomes if n!='link_ete']
#     if bpergunta:
#         print(f'removeTabelasTemporarias. O script vai fazer altera��es no banco de dados, APAGANDO as tabelas {tabelasAApagar}. Prossegue?(y,n)')
#         if input()!='y':
#             exit()
...
if __name__ == "__main__":
    ...
    if os.path.exists(camDBSaida):
        print(f'O arquivo {camDBSaida} j� existe. Apague primeiro e tente novamente.')
        sys.exit(0)
    print('O script vai fazer altera��es no banco de dados. A execu��o leva cerca de 3.5 horas. Prossegue?(y,n)')
    if input()!='y':
        exit()
    ...
    ##compacta(camDBSaida) #isso est� muito devagar
